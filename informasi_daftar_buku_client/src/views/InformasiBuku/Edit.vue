...
export default {
  setup(){
    ...
    onMounted(() => {
      axios.get(`http://127.0.0.1:8000/api/buku/${route.params.id}`)
      .then((result)=> {
        informasi_buku.judul = result.data.data.judul,
        informasi_buku.isi = result.data.data.isi,
        informasi_buku.id_penulis = result.data.data.id_penulis,
        informasi_buku.halaman = result.data.data.halaman,
        informasi_buku.gambar = result.data.data.gambar
      }).catch((err) => {
    ...
    function update() {
      axios.put(
        `http://127.0.0.1:8000/api/buku/${route.params.id}`,
        informasi_buku
      )
      .then(() => {
          router.push({
